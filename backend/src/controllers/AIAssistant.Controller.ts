...
export class AIAssistantController {
  ...
  async getProject(projectName: string): Promise<ProjectType> {
    let project = await this.databaseService.projectService.getProjectByName(projectName);
    if (project == null) {
      project = await this.databaseService.projectService.createProject(projectName);
    }
  ...
  async getDiscussion(user: UserType, project: ProjectType): Promise<DiscussionType> {
    let discussion = await this.databaseService.discussionService.getDiscussion(user, project);
    if (discussion == null) {
      discussion = await this.databaseService.discussionService.createDiscussion(user, project);
    }
